...
class AnnotatedSpectrumBatch(NamedTuple):
    ...
    spectrum_index: torch.IntTensor
...
class PolarsSpectrumDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, float, float]:
        ...
        spectrum_index = row["index"].to_numpy()[0]

        return spectrum, precursor_mz, precursor_charge, spectrum_index
...
class AnnotatedPolarsSpectrumDataset(PolarsSpectrumDataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, float, float, str]:  # type: ignore[override]
        spectrum, precursor_mz, precursor_charge, spectrum_index = super().__getitem__(idx)
        peptide = self.peptides[idx]

        return spectrum, precursor_mz, precursor_charge, peptide, spectrum_index
...
def collate_batches(
    ...
    def fn(
        ...
        if annotated:
            spectra, precursor_mz, precursor_charge, peptides, spectrum_index = list(zip(*batch))
        else:
        ...
        if annotated:
            ...
            return AnnotatedSpectrumBatch(
                spectra, spectra_padding_mask, precursors, peptides, peptide_padding_mask, spectrum_index
            )
        else:
