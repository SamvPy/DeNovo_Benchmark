...
def main(args):
    ...
    filename = os.path.basename(args.result_path).split(".")[0]
    ...
    df["modified_sequence"] = df["modified_sequence"].apply(
        proforma_to_instanovo_mod
    )
    ...
    index_map = df[["index", "title"]].set_index("index").to_dict("index")
    ...
    df.to_feather(os.path.join(args.output_folder, filename) + ".feather")


if __name__ == "__main__":

    parser = argparse.ArgumentParser(description="Write mgf files, annotated by a given de novo search")
    parser.add_argument('-r', '--result_path', required=True, help="Path to result file generated by a de novo search engine.")
