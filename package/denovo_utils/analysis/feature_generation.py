...
def calculate_hyperscore(
    ...
    ----------
    psm: psm_utils.PSM
        The PSM used to extract 'spectrum_id' (for MGF spectrum extraction) and 'Peptidoform' (the peptide sequence)
    mgf_file: pyteomics.mgf.IndexedMGF
        An mgf-file (or mzml) used to extract the spectrum (based on 'spectrum_id' in PSM and 'id' in the file)
    ...
    observed_spectrum_oms.set_peaks(
        [
            observed_spectrum["m/z array"],
            observed_spectrum["intensity array"]
        ]
    )
