"""Analytic functionalities for metaproteomics analyses."""

import pandas as pd
import seaborn as sns

from ..parsers import DenovoEngineConverter
from ..utils.pandas import count_species_peptides, get_psm_type, get_species_specificity


def count_analysis(
    path_file: str, psm_types: list, fasta_dict: dict, engine="sage", plot=True
):
    """
    ...
    psm_types: list[str]
        list with 'decoy', 'target_rejected', 'target_accepted',
        dependent on which subset
        should be included
    fasta_dict: dict
    ...
    df_sage = parser.parse(result_path=path_file, mgf_path="").to_dataframe()
    ...
    df_sage_filtered["species"] = df_sage_filtered.progress_apply(
        lambda x: get_species_specificity(x["protein_list"], fasta_dict), axis=1
    )
    ...
    df_sage_unique_species["species"] = df_sage_unique_species["species"].apply(
        lambda x: x[0]
    )

    if plot:
        sns_plot = sns.kdeplot(
            df_sage_unique_species, x="score", hue="species", common_norm=False
        )
